...
in
{
  ...
  config = lib.mkIf cfg.enable {
    rice.backup.command = pkgs.writeShellScript "restic-backup" ''
      set -uo pipefail
      REPO="${cfg.repository}"
      [ -d "$REPO" ] || { echo "[$(${pkgs.coreutils}/bin/date)] skip: $REPO not mounted"; exit 0; }

      # single-flight: RunAtLoad + StartOnMount + daily timer can overlap on one repo
      LOCK="/tmp/restic-backup.lock"
      ${pkgs.coreutils}/bin/mkdir "$LOCK" 2>/dev/null || { echo "[$(${pkgs.coreutils}/bin/date)] another backup running; skip"; exit 0; }
      trap '${pkgs.coreutils}/bin/rmdir "$LOCK" 2>/dev/null || true' EXIT
      # launchd stop / logout delivers TERM; untrapped it kills bash WITHOUT running the
      # EXIT trap, and the stale lock turns every later run into a silent skip
      ...
      notify() {
        # /usr/bin/osascript is OS-fixed on darwin and doubles as the platform gate.
        # notify-send stays a PATH probe so this shared script does not drag libnotify
        # into the darwin closure.
        [ -x /usr/bin/osascript ] \
          && /usr/bin/osascript -e "display notification \"$1\" with title \"restic backup\"" >/dev/null 2>&1 && return
      ...
      echo "[$(${pkgs.coreutils}/bin/date)] backing up: ${lib.escapeShellArgs cfg.paths}"
      if ! "''${RUNITOR[@]}" "$RESTIC" backup ${lib.escapeShellArgs cfg.paths} ${
        lib.concatMapStringsSep " " (e: "--exclude=${lib.escapeShellArg e}") cfg.exclude
      }; then
        echo "[$(${pkgs.coreutils}/bin/date)] BACKUP FAILED"
        notify "easystore backup FAILED: check the log"
        exit 1
      fi
      "$RESTIC" unlock 2>/dev/null || true
      "$RESTIC" forget ${lib.escapeShellArgs cfg.pruneOpts} --prune || echo "[$(${pkgs.coreutils}/bin/date)] prune warning (non-fatal)"

      # weekly structural integrity check, marker-gated
      CHECKMARK="$HOME/.cache/restic-lastcheck"
      ${pkgs.coreutils}/bin/mkdir -p "$HOME/.cache"
      if [ ! -e "$CHECKMARK" ] || [ -n "$(${pkgs.findutils}/bin/find "$CHECKMARK" -mtime +7 2>/dev/null)" ]; then
        echo "[$(${pkgs.coreutils}/bin/date)] weekly restic check"
        if "$RESTIC" check; then
          ${pkgs.coreutils}/bin/touch "$CHECKMARK"
        else
          echo "[$(${pkgs.coreutils}/bin/date)] restic check FAILED: repo may be corrupt"
      ...
      if [ ! -e "$REHEARSEMARK" ] || [ -n "$(${pkgs.findutils}/bin/find "$REHEARSEMARK" -mtime +7 2>/dev/null)" ]; then
        echo "[$(${pkgs.coreutils}/bin/date)] weekly restore rehearsal"
        # smallest non-empty file across the paths, NUL-safe walk. stat is pinned to
        # GNU coreutils, so the -c form is right on both platforms (no BSD probe)
        REH_FILE=""
        REH_SIZE=""
        while IFS= read -r -d "" f; do
          sz=$(${pkgs.coreutils}/bin/stat -c '%s' "$f") || continue
          if [ -z "$REH_SIZE" ] || [ "$sz" -lt "$REH_SIZE" ]; then
            REH_SIZE="$sz"
            REH_FILE="$f"
          fi
        done < <(${pkgs.findutils}/bin/find ${lib.escapeShellArgs cfg.paths} -type f -size +0c -print0 2>/dev/null)

        if [ -z "$REH_FILE" ]; then
          echo "[$(${pkgs.coreutils}/bin/date)] rehearsal: no non-empty file found; skipping"
          ${pkgs.coreutils}/bin/touch "$REHEARSEMARK"
        else
          REH_DIR="$(${pkgs.coreutils}/bin/mktemp -d "''${TMPDIR:-/tmp}/restic-rehearsal.XXXXXX")"
          trap '${pkgs.coreutils}/bin/rm -rf "$REH_DIR" 2>/dev/null || true; ${pkgs.coreutils}/bin/rmdir "$LOCK" 2>/dev/null || true' EXIT
          if "$RESTIC" restore latest --target "$REH_DIR" --include "$REH_FILE" --verify >/dev/null 2>&1; then
            REH_OUT="$(${pkgs.findutils}/bin/find "$REH_DIR" -type f -size +0c -print -quit 2>/dev/null)"
            if [ -z "$REH_OUT" ] || [ ! -s "$REH_OUT" ]; then
              echo "[$(${pkgs.coreutils}/bin/date)] rehearsal: restored file missing or empty"
              notify "restore rehearsal FAILED: restored file empty/missing"
            elif [ -e "$REH_FILE" ] && ! ${pkgs.diffutils}/bin/cmp -s "$REH_OUT" "$REH_FILE"; then
              echo "[$(${pkgs.coreutils}/bin/date)] rehearsal: restored bytes differ from live source"
              notify "restore rehearsal FAILED: content mismatch"
            else
              echo "[$(${pkgs.coreutils}/bin/date)] rehearsal OK ($REH_SIZE bytes round-tripped)"
              ${pkgs.coreutils}/bin/touch "$REHEARSEMARK"
            fi
          else
            echo "[$(${pkgs.coreutils}/bin/date)] rehearsal: restic restore failed"
            notify "restore rehearsal FAILED: restore errored"
          fi
          ${pkgs.coreutils}/bin/rm -rf "$REH_DIR" 2>/dev/null || true
          trap '${pkgs.coreutils}/bin/rmdir "$LOCK" 2>/dev/null || true' EXIT
        fi
      fi
      echo "[$(${pkgs.coreutils}/bin/date)] done"
    '';
  };
}
