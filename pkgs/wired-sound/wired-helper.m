...
// session. on each UNLOCK it afplays the (low, slightly-wrong) unlock tone. on
// SIGTERM (logout/shutdown tears the agent down) it logs the end-card once. NO TTS:
// the macOS `say` voice was removed (it read uncanny). it deliberately does NOT react
// to lock: the machine answers your return, it does not announce your leaving.
//
